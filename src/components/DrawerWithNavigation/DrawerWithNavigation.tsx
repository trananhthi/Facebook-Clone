...
const ButtonInDrawer = ({ title, to, children }: { title: string; to: string; children: React.ReactNode }) => {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        classNames('w-[160px] h-[56px] flex justify-center items-center', {
          'border-b-4 border-blue-700': isActive
        })
      }
    >
      <Button variant='text' className=' flex justify-center items-center gap-2 w-[160px]'>
        {children}
        <div className={`text-[13px]  ${location.pathname === to ? 'text-blue-500' : 'text-primary'}`}>{title}</div>
      </Button>
    </NavLink>
  )
}
...
export const DrawerWithNavigation = ({ isOpenDrawer, setIsOpenDrawer }: DrawerWithNavigationProps) => {
  return (
    <Drawer size={250} open={isOpenDrawer} onClose={() => setIsOpenDrawer(false)} className='p-4 shadow '>
      <div className='mb-4 flex items-center justify-between border-b border-gray-300'>
        <Typography variant='h5' color='blue'>
          Điều hướng
        </Typography>
        <IconButton variant='text' color='blue-gray' onClick={() => setIsOpenDrawer(false)}>
          <svg
      ...
      <div className='flex flex-col items-center gap-4'>
        <ButtonInDrawer title='Trang chủ' to={routes.home}>
        ...
        <ButtonInDrawer title='Bạn bè' to={routes.friend}>
        ...
        <ButtonInDrawer title='Nhóm' to={routes.group}>
